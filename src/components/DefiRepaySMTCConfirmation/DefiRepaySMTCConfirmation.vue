<template>
    <div class="defi-repay-smtc-confirmation">
        <tx-confirmation
        ...
        >
            ...
            <div class="confirmation-info" tabindex="0" data-focus>
                <template v-if="d_step === 1"
                    >You’re allowing {{ parseFloat(dOutstandingSMTC).toFixed(2) }} sMTC</template
                >
                <template v-else>You’re repaying {{ parseFloat(dOutstandingSMTC).toFixed(2) }} sMTC</template>
            </div>
...
import sfcUtils from 'metatech-ledgerjs/src/sfc-utils.js';
import erc20Utils from 'metatech-ledgerjs/src/erc20-utils.js';
import Web3 from 'web3';
import appConfig from '../../../app.config.js';

export default {
    name: 'DefiRepaySMTCConfirmation',
    ...
    data() {
        return {
            ...
            d_outstandingSMTC: this.outstandingSMTC,
            d_step: this.step,
    ...
    computed: {
        ...
        hasCorrectParams() {
            return !!this.d_stakerId && !!this.d_outstandingSMTC;
        },
        ...
        passwordLabel() {
            ...
            if (this.d_step === 1) {
                ...
                label = 'Please enter your wallet password to mint sMTC';
            }
    ...
    methods: {
        async setTx() {
            ...
            if (!this.smtcToken || !$defi.contracts.StakeTokenizerContract) {
                return;
            }

            this.dOutstandingSMTC = $defi.fromTokenValue(this.d_outstandingSMTC, this.smtcToken);
            ...
            if (this.d_step === 1) {
                txToSign = erc20Utils.erc20IncreaseAllowanceTx(
                    this.smtcToken.address,
                    $defi.contracts.StakeTokenizerContract,
                    Web3.utils.toHex(
                        $defi.shiftDecPointRight((this.dOutstandingSMTC * 1.05).toString(), this.smtcToken.decimals)
                    )
                );
            } else {
        ...
        onSendTransactionSuccess(_data) {
            if (this.d_step === 1) {
                this.$emit('change-component', {
                    to: 'staking-repay-smtc-confirmation-success-message',
                    from: this.compName,
                    data: {
                        ...
                        continueToParams: {
                            ...
                            outstandingSMTC: this.d_outstandingSMTC,
                            tmpPwdCode: this.d_tmpPwdCode,
                        },
                        continueTo: 'staking-repay-smtc-confirmation2',
                ...
                this.$emit('change-component', {
                    to: 'staking-repay-smtc-confirmation-success-message2',
                    from: this.compName,
                    data: {
                        tx: _data.data.sendTransaction.hash,
