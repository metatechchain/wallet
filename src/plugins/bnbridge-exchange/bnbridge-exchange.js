...
export class BNBridgeExchange {
    ...
    /**
     * Get metatech token from tokens list.
     *
     * @return {BNBridgeToken|undefined}
     */
    async getMetaTechToken() {
    ...
    async getETHBalance(_ethReceiveAddress, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getMetaTechToken());
        const pData = {
     ...
     * @param {string} [metatechAddress]
     * @param {BNBridgeDirection} [direction]
     * @param {BNBridgeToken} [_token]
     * @return {Promise<null>}
     */
    async swapToken({ ethAddress = '', bnbAddress = '', metatechAddress = '', direction = 'EthereumToBinance' }, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getMetaTechToken());
        const pData = {
            direction,
            token_uuid: token ? token.uuid : '',
            ...
            metatech_address: metatechAddress,
        };
            ...
            if (data) {
                if (data.success) {
                    ...
                    if (
                        ...
                        pData.metatech_address !== pData.metatech_address.toLowerCase()
                    ) {
                        // resend request with MetaTech address in lowercase
                        pData.metatech_address = pData.metatech_address.toLowerCase();
     ...
     * @return {string}
     */
    getMTCCurrencyByDirection(_direction) {
        let currency = 'MTC';

        if (_direction === 'MetaTechToMetaTech') {
            currency += '-MetaTech';
