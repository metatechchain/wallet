...
export default {
    ...
    methods: {
        async init() {
            ...
            const wMTC = result[1].filter((_token) => _token && _token.canWrapMTC);
            ...
            const mtcToken = {
                ...
                decimals: 18,
                price: wMTC[0].price,
                priceDecimals: wMTC[0].priceDecimals,
                availableBalance: account.balance,
                allowance: '0x0',
                logoUrl: 'https://cryptologos.cc/logos/metatech-mtc-logo.svg?v=003',
            };
            this.$defi._setTokenDecimals(mtcToken);

            // add MTC
            result[1].unshift(mtcToken);

            this.tokens = result[1].filter((_token) => _token && (_token.symbol === 'MTC' || _token.canWrapMTC));
        ...
        onSubmit() {
            ...
            if (fromToken.canWrapMTC && toToken.symbol === 'MTC') {
                this.stepsCount = 2;
