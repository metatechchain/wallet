                ...
                <template v-if="isMMInstalled">
                    <div class="mmsettings_section">
                        ...
                        <button
                            id="add_mainnet_btn"
                            class="btn large"
                            :disabled="addMetaTechMainnetInProgress"
                            @click="onAddChainClick('mainnet')"
                        >
                            Add Meta Tech Mainnet
                            <pulse-loader v-if="addMetaTechMainnetInProgress" color="#fff"></pulse-loader>
                        </button>
                        <button
                            id="add_testnet_btn"
                            class="btn large secondary"
                            :disabled="addMetaTechTestnetInProgress"
                            @click="onAddChainClick('testnet')"
                        >
                            Add Meta Tech Testnet
                            <pulse-loader v-if="addMetaTechTestnetInProgress" color="#1969ff"></pulse-loader>
                        </button>
                    </div>
...
import { METATECH_MAINNET, METATECH_TESTNET } from '@/plugins/mm/mm.js';
...
export default {
    ...
    data() {
        return {
            ...
            popoverText: '',
            btnId: '',
            addMetaTechMainnetInProgress: false,
            addMetaTechTestnetInProgress: false,
    ...
    methods: {
        ...
        stopLoadingIndicators() {
            this.addMetaTechMainnetInProgress = false;
            this.addMetaTechTestnetInProgress = false;
        ...
        async onAddChainClick(_type = 'mainnet') {
            ...
            const chain = mainnet ? METATECH_MAINNET : METATECH_TESTNET;
            const btnId = mainnet ? 'add_mainnet_btn' : 'add_testnet_btn';

            if (mainnet) {
                this.addMetaTechMainnetInProgress = true;
            } else {
                this.addMetaTechTestnetInProgress = true;
            }
