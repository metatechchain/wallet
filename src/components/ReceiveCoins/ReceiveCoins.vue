<template>
    <div>
        <h2 class="align-center" data-focus>Receive MetaTech MTC</h2>
...
import ReceiveMTC from './ReceiveMTC.vue';
...
export default {
    ...
    components: { BlockchainPicker, FCard, ReceiveMTC, ReceiveBNB, ReceiveETH, TransactionCompleting },

    mixins: [eventBusMixin],

    props: {
        /** Start verify MTC account in ReceiveMTC component */
        verifyAccount: {
            type: Boolean,
            default: false,
        },
    ...
    methods: {
        /**
         * @param {('metatech'|'binance'|'ethereum')} _blockchain
         */
        onBlockchainPick(_blockchain) {
            switch (_blockchain) {
                case 'metatech':
                    this.currentComponent = 'ReceiveMTC';
                    break;
                case 'binance':
