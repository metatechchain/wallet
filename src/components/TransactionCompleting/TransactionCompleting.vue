        ...
        <template v-if="success">
            ...
            <ul class="no-markers" style="margin-bottom: 16px;">
                <li v-for="result in tsData.result" :key="result.uuid">
                    <b>
                        {{ result.amount }}
                        {{ $bnb.getMTCCurrencyByDirection(receive ? tsData.direction : 'MetaTechToMetaTech') }}
                    </b>
                    from
                    <b class="break-word">{{
                        receive ? tsData[$bnb.getAddressKeyByDirection(tsData.direction)] : tsData.from_metatech_address
                    }}</b>
                </li>
            </ul>
            ...
            <p>
                ...
                <b>
                    ...
                    {{ $bnb.getMTCCurrencyByDirection(receive ? 'MetaTechToMetaTech' : tsData.direction) }}
                </b>
                in your address
                <b> {{ tsData.address || tsData.result[0].metatech_address }}</b>
            </p>
