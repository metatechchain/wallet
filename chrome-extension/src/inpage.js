 ...
 */
class MetaTechInpageProvider extends MetaTechEventEmitter {
    ...
    _sendToContentScript(payload, callback = null) {
        ...
        if (this.debug) console.log('MetaTechPwaWallet request', id, payload);
        window.postMessage({
            target: 'MetaTechPWAwalletBackground',
            data: payload,
            msgId: id,
        }, location.origin);
    }
    ...
    _receiveFromContentScript(event) {
        ...
        if (msg.target !== 'MetaTechPWAwalletInpage') return
        if (!msg.data) return
        let data = msg.data;

        if (this.debug) console.log('MetaTechPwaWallet response', msg.msgId, data);
        ...
        if (msg.msgId && typeof this.messageCallbacks[msg.msgId] !== 'undefined') {
            ...
            if (data.error) {
                console.log('MetaTechPwaWallet RPC error', data);
                callback(data.error, null); // (error, result)
            } else {
                callback(null, data); // (error, result)
            }
            if (this.debug) console.log('MetaTechPwaWallet response callback called', msg.msgId, data);
        }
    ...
    _handleAccountsChanged (accounts, isEthAccounts = false, isInternal = false) {
        if (this.debug) console.log('MetaTechPwaWallet accountsChanged', accounts);
        this.emit('accountsChanged', accounts)
    }
}

window.ethereum = new MetaTechInpageProvider();
