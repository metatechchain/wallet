<template>
    <div ref="doc" class="stake-mtc" tabindex="0">
                    ...
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                                    -
                                    <!--{{ toMTC(accountInfo.claimedRewards) }} MTC-->
                                </template>
                            </f-placeholder>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        <div class="col f-row-label">Minted sMTC</div>
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                        ...
                        <template v-if="accountInfo && accountInfo.delegated !== '0x0'">
                            <template v-if="accountInfo && accountInfo.preparedForWithdrawal && false">
                                ...
                                <h3 class="align-center">
                                    Your {{ toMTC(accountInfo.delegated) }} MetaTech MTC is available for withdraw in 7 days.
                                </h3>
                                -->
                            </template>
                            <template v-else>
                                ...
                                <f-message v-if="showRepaySMTCMessage" type="info" with-icon class="align-start">
                                    Can't repay sMTC, not enough unlocked sMTC
                                </f-message>
                                <f-message v-if="showUndelegateMessage" type="info" with-icon class="align-start">
                                    Can't undelegate, repay sMTC first please
                                </f-message>
                            </template>
                        </template>
                        <!--                        <template v-else>
        ...
        >
            ...
            <f-message
                v-if="outstandingSMTC > 0"
                type="warning"
                with-icon
                class="align-start"
                style="margin-bottom: 16px;"
            >
                Can't withdraw, repay sMTC first please
            </f-message>

            <withdraw-request-list
                :disable-withdraw="outstandingSMTC > 0"
...
import { toMTC, WeiToMTC } from '../../utils/transactions.js';
...
import { SFC_CLAIM_MAX_EPOCHS } from '@/plugins/metatech-web3-wallet.js';
...
*/
        ...
        canRepaySMTC() {
            return (
                // this.canUndelegate &&
                this._delegation &&
                this._delegation.outstandingSMTC !== '0x0' &&
                this.outstandingSMTC <= this.availableSMTC
            );
        },
        ...
        outstandingSMTC() {
            return this.smtcToken && this._delegation
                ? this.$defi.fromTokenValue(this._delegation.outstandingSMTC, this.smtcToken) || 0
                : 0;
        },
        ...
        withdrawRequestsAmount() {
            ...
            if (delegation && delegation.withdrawRequests && delegation.withdrawRequests.length) {
                delegation.withdrawRequests.forEach((_request) => {
                    if (delegation.toStakerId === _request.stakerID) {
                        amount += WeiToMTC(_request.amount);
                    }
                });
            }
    ...
    methods: {
        async init() {
            ...
            this.smtcToken = result[0].find((_item) => _item.symbol === 'SMTC') || {};
        },
        ...
        mintSMTC() {
            ...
            this.showConfirmationWindow({
                ...
                windowTitle: 'Mint sMTC',
            });
        },

        repaySMTC() {
            if (!this.canRepaySMTC) {
                return;
            }
        ...
        async onWithdrawRequestSelected(_withdrawRequest) {
            ...
            this.showConfirmationWindow({
                ...
                data: {
                    ...
                    amount: WeiToMTC(_withdrawRequest.amount),
                    withdraw: true,
