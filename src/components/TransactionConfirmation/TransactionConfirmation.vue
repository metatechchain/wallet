    ...
    <div class="transaction-confirmation">
        ...
        >
            <h2 class="not-visible" data-focus>
                ...
                <span v-else>Send MetaTech MTC</span>
            </h2>
            ...
            <div class="transaction-info">
                ...
                <template v-if="sendDirection !== 'MetaTechToMetaTech'">
                    <f-message type="warning" class="align-center">
                        All bridge transactions incur a fee of {{ minMTCToTransfer }} MTC, deducted from the transfer
                        amount.
                    </f-message>
                    <f-message type="info" class="big">
                        You will receive <b>{{ txData.amount - minMTCToTransfer }} MTC</b>
                    </f-message>
                    <br />
                </template>
            ...
            <template #window-content>
                        ...
                        <div class="row no-collapse">
                            ...
                            <div class="col break-word">
                                ...
                                <span class="f-row-label">
                                    ...
                                    ( {{ toMTC(currentAccount.balance) }} MTC
                                    <template v-if="currentAccount.name">, {{ currentAccount.name }}</template> )
                                </span>
                            </div>
                        </div>
                ...
                <f-message v-if="sendDirection === 'MetaTechToEthereum'" type="warning" class="align-center">
                    All bridge transactions incur a fee of {{ minMTCToTransfer }} MTC, deducted from the transfer
                    amount.
                    <br />
                    You will receive {{ txData.amount - minMTCToTransfer }} MTC
                    <br />
                </f-message>
            </template>
        </tx-confirmation>
...
import { Web3 } from '../../plugins/metatech-web3-wallet.js';
import { toMTC } from '../../utils/transactions.js';
import { formatNumberByLocale } from '../../filters.js';
import TxConfirmation from '../TxConfirmation/TxConfirmation.vue';
import erc20Utils from 'metatech-ledgerjs/src/erc20-utils.js';
...
export default {
    components: { MTCTokenValue, FMessage, FTokenValue, TxConfirmation },
    ...
    asyncComputed: {
        async sendToAddressBalance() {
            ...
            } else if (sendDirection === 'MetaTechToMetaTech') {
                data = await this.$fWallet.getBalance(this.txData.metatech_address);
                balance = this.toMTC(data.balance);
            } else if (sendDirection === 'MetaTechToBinance') {
            ...
            } else if (sendDirection === 'MetaTechToEthereum') {
    ...
    methods: {
        async setSendToAddress() {
            ...
            if (sendDirection === 'MetaTechToMetaTech') {
                this.sendToAddress = this.txData.metatech_address;
            } else if (sendDirection === 'MetaTechToBinance') {
                stData = {
            ...
            if (stData) {
                ...
                this._swapTokenData = {
                    from_metatech_address: this.currentAccount.address,
                    ...this.txData,
                    ...data,
                };
                this.sendToAddress = data.metatech_address;
                this.dTxData.metatech_address = data.metatech_address;
            ...
            if (token.address) {
                this.tx = await fWallet.getDefiTransactionToSign(
                    erc20Utils.erc20TransferTx(
                        token.address,
                        fWallet.toChecksumAddress(dTxData.metatech_address),
                ...
                this.tx = await fWallet.getTransactionToSign({
                    ...
                    from,
                    to: fWallet.toChecksumAddress(dTxData.metatech_address),
                    memo: dTxData.memo,
                });
            }
        },
