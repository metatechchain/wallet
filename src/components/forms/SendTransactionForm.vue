<template>
    <div class="send-transaction-form">
        <h2 :id="labelId" class="with-back-btn align-center" data-focus>
            ...
            <template v-else>Send MetaTech MTC</template>
                ...
                <fieldset class="">
                    <div class="form-body">
                        ...
                        <address-field
                            ...
                            :validate-on-input="d_sendDirection === 'MetaTechToMetaTech'"
                        >
                            <template #bottom="sProps">
                                <f-message v-show="sProps.showErrorMessage" type="error" alert with-icon>
                                    {{ sendToErrorMsg }}
                        ...
                        <f-input
                            v-if="d_sendDirection !== 'MetaTechToEthereum' && !token.address"
                            label="Memo (optional)"
                            field-size="large"
                            name="memo"
                        />
                        ...
                        <div class="align-center form-buttons">
                            <template v-if="d_sendDirection !== 'MetaTechToMetaTech'">
                                <f-message type="warning" class="align-center">
                                    All bridge transactions incur a fee of {{ minMTCToTransfer }} MTC, deducted from the
                                    transfer amount.
                                </f-message>
                                <f-message
                                    v-if="amount >= minMTCToTransfer && checkAmount(amount)"
                                    type="info"
                                    class="big"
                                >
                                    You will receive <b>{{ amount - minMTCToTransfer }} MTC</b>
                                </f-message>
                                <br />
                            </template>
...
export default {
    ...
    data() {
        return {
            d_sendDirection: 'MetaTechToMetaTech',
            amountErrMsg: 'Invalid amount',
            gasPrice: '',
            amount: '',
            sendToErrorMsg: 'Enter a valid MetaTech MTC address or domain name',
            /** Balance of BNB or ETH account. */
            ETHOrBNBAccountBalance: '',
            minMTCToTransfer: appConfig.bnbridgeApi.minMTCToTransfer,
    ...
    computed: {
        ...
        sendToLabel() {
            ...
            switch (this.d_sendDirection) {
                case 'MetaTechToBinance':
                    sendTo = 'BNB Receive Address';
                    break;
                case 'MetaTechToEthereum':
    ...
    methods: {
        ...
        async checkAddress(_value) {
            ...
            if (d_sendDirection === 'MetaTechToMetaTech') {
                value = (await this.resolveAddress(value, 'MTC', 'METATECH')) || value;
                validAddress = this.$fWallet.isValidAddress(value);
                this.sendToErrorMsg = 'Enter a valid MetaTech MTC address or domain name';
            } else if (d_sendDirection === 'MetaTechToBinance') {
            ...
            } else if (d_sendDirection === 'MetaTechToEthereum') {
        ...
        checkAmount(_value) {
            ...
            const { minMTCToTransfer } = this;
            const metatechToBridge = this.d_sendDirection !== 'MetaTechToMetaTech';
            ...
            if (!isNaN(value)) {
                if (value <= remainingBalance && value > 0) {
                    if (metatechToBridge && value < minMTCToTransfer) {
                        this.amountErrMsg = `You must transfer at least ${minMTCToTransfer} ${tokenSymbol}`;
                    } else {
                        ok = true;
                    }
        ...
        */
        async onFormSubmit(_event) {
            ...
            if (this.currentAccount && data.amount) {
                if (d_sendDirection === 'MetaTechToMetaTech') {
                    data.metatech_address = this.resolvedAddress || data.address;
                } else if (d_sendDirection === 'MetaTechToBinance') {
                ...
                this.$emit('change-component', {
                    ...
                    data: {
                        ...
                        amount: '0.1',
                        fee: '0.000044',
                        from_metatech_address: '0x76AE07E6D236c1aE3F5C3112F387ad82c69A2471',
                        memo: '',

                        bnb_address: 'bnb1jvlepaalght59kwhfh44k54u3elmhuurnt0lxc',
                        direction: 'MetaTechToBinance',
                        metatech_address: '0xE504aF2999644A86162Df892E86E3809a365AEBa',
...
*/
                ...
                /*this.$emit('change-component', {
                    ...
                    data: {
                        ...data,
                        // fee: this.$fWallet.WEIToMTC(this.$fWallet.getTransactionFee(this.gasPrice)),
                    },
                });*/
            }
        },
