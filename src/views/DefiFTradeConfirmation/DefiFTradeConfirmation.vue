    ...
    <div class="defi-ftrade-confirmation">
        ...
        >
            ...
            <template #window-content>
                <ledger-confirmation-content :to="tx.to" :amount="amountMTC" :max-fee="tx._fee" />
            </template>
        </tx-confirmation>
        <template v-else>
...
import wmtcUtils from 'metatech-ledgerjs/src/wmtc-utils.js';
        ...
        async setTx() {
            ...
            */
            ...
            if (this.params.step === 1) {
                ...
                if (fromToken.symbol === 'MTC' && toToken.canWrapMTC) {
                    this.amountMTC = params.fromValue.toFixed(this.$defi.getTokenDecimals(params.fromToken));

                    txToSign = wmtcUtils.defiWrapMTC(
                        toToken.address,
                        Web3.utils.toHex(this.$defi.shiftDecPointRight(params.toValue.toString(), toToken.decimals))
                    );
                } else if (fromToken.canWrapMTC && toToken.symbol === 'MTC') {
                    const amount = Web3.utils.toHex(
                        this.$defi.shiftDecPointRight(params.fromValue.toString(), toToken.decimals)
                    );

                    txToSign = wmtcUtils.defiUnwrapMTC(
                        fromToken.address,
