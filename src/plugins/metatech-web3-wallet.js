...
export const METATECH_CHAIN_ID = 0x93c;
...
// Meta Tech web3 wallet plugin for VUE, based on https://git`hub.com/Meta Tech-io/metatech-web3-wallet
export class MetaTechWeb3Wallet {
    static install(_Vue, _options) {
        fWallet = new MetaTechWeb3Wallet(_options);
    ...
    async getTransactionToSign({ from, to, value, memo = '' }) {
        ...
        return {
            ...
            _fee: !error ? this.WEIToMTC(this.getTransactionFee(gasPrice, gasLimit)) : 0,
        };
    }
    ...
    */
    ...
    getRemainingBalance(_balance, _gasPrice, _gasLimit) {
        ...
        return parseFloat(this.WEIToMTC(balance.sub(fee.mul(this.toBN(2)))));
    }
    ...
    getMaxRemainingBalance(_balance, _gasPrice) {
        ...
        return parseFloat(this.WEIToMTC(balance.sub(fee.mul(this.toBN(1)))));
    }
    ...
    /**
     * @param _address
     * @param {('metatech' | 'ethereum' | 'binance')} _blockchain
     * @return {boolean}
     */
    isValidAddress(_address, _blockchain = 'metatech') {
        if (_blockchain === 'metatech' || _blockchain === 'ethereum') {
